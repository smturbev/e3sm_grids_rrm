...
# for uniform grid dst_grd=exodus_ne${NE_DST}.g
# gives an OOM error if --mpi_nbr=16 (matches SBATCH) is used below
# the command below took ~ 9 hrs wallclock time
time ncremap ${MAP_ARGS} -a fv2se_flx \
  --src_grd=${grid_root}/scrip_ne${NE_SRC}pg1.nc \
  --dst_grd=${scrip_dst_grid} \
...
#SBATCH --mail-user=smturbe@sandia.gov
#SBATCH --mail-type=ALL
#SBATCH --output=zstep4c_homme_tool_input1.o
#SBATCH --error=zstep4c_homme_tool_input1.e

srun -K -c 1 -N 1 ${homme_tool_root}/src/tool/homme_tool < input.nl
EOF

# Uncomment to run homme_tool with grid_template_tool for GLL scrip
sbatch homme_tool_inputnl.sh
exit 1
...
ncks -O -v lat,lon,area,cv_lat,cv_lon ne0np4_tmp1.nc ${grid_name}_tmp.nc

# load homme-env (check .bashrc) before running
...
ncl ${e3sm_root}/components/homme/test/tool/ncl/HOMME2SCRIP.ncl name=\"${grid_name}\" ne=0 np=4
exit 1
